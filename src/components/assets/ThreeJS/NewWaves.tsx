...
// The Line component that generates the waves
function Lines({ amplitude, frequency }: { amplitude: number; frequency: number }) {
...
function AnimationCanvas({ amplitude, frequency }: { amplitude: number; frequency: number }) {
  return (
    <div style={{ zIndex: '1', position: 'relative', width: '100vw', height: '100vh', backgroundColor: '#121212' }}>
      ...
      >
        <Suspense fallback={<div>Loading...</div>}>
          <Lines amplitude={amplitude} frequency={frequency} />
        </Suspense>
        <OrbitControls />
      </Canvas>
...
export default function Waves() {
  ...
  return (
    <Flex direction="column" align="center" justify="center" bg="#121212" color="white" p={6}>
      <Text fontSize="2xl" mb={6}>Interactive Wave Animation</Text>
      ...
      <Box mb={6} width="300px">
        ...
        <Slider value={frequency} min={0.0001} max={0.001} step={0.00001} onChange={setFrequency}>
          <SliderTrack>
            <SliderFilledTrack />
          </SliderTrack>
          <SliderThumb />
      ...
      <AnimationCanvas amplitude={amplitude} frequency={frequency} />
    </Flex>
  );
}
